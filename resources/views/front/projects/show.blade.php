        ...
        <section class="work-details-area">
          <div class="work-details-area-inner section-spacing">
            <div class="container large">
              <div class="section-header fade-anim">
                <div class="section-title-wrapper">
                  <div class="title-thumb">
                    <img src="{{ $project->featured_image ? asset('storage/'.$project->featured_image) : asset('assets/imgs/gallery/image-26.webp') }}" alt="{{ $project->title }}">
                  </div>
                  <div class="title-wrapper">
                    <h2 class="section-title font-sequelsans-romanbody">{{ $project->title }}</h2>
                  </div>
                </div>
              </div>
              <div class="meta-wrapper fade-anim">
                <div class="meta-item">
                  ...
                  <p class="text">{{ $project->service ?: '-' }}</p>
                </div>
                <div class="meta-item">
                  <p class="title">Client</p>
                  <p class="text">{{ $project->client ?: '-' }}</p>
                </div>
                <div class="meta-item">
                  <p class="title">Date</p>
                  <p class="text">{{ $project->created_at?->format('F Y') ?: '-' }}</p>
                </div>
                <div class="meta-item">
                  <p class="title">Technology</p>
                  <p class="text">{{ $project->technology ?: '-' }}</p>
                </div>
              </div>
            </div>
            <div class="image-wrapper parallax-view fade-anim">
              <img class="w-100" src="{{ $project->featured_image ? asset('storage/'.$project->featured_image) : asset('assets/imgs/gallery/image-27.webp') }}" alt="{{ $project->title }}" data-speed="0.8">
            </div>
            <div class="container large">
              <div class="section-info fade-anim">
                ...
                <div class="content">
                  <div class="text-wrapper">
                    <div class="text">{!! $project->description ?: 'Project details will be available soon.' !!}</div>
                  </div>
                  <div class="feature-list">
                    <ul>
                      @forelse(collect(explode(',', (string) $project->technology))->map(fn ($item) => trim($item))->filter() as $tech)
                        <li>{{ $tech }}</li>
                      @empty
                        <li>{{ $project->service ?: 'Service' }}</li>
                      @endforelse
                    </ul>
                  </div>
                </div>
            ...
            <div class="gallery-wrapper fade-anim">
              @if(!empty($project->gallery))
                ...
                @endforeach
              @else
                <div class="image parallax-view"><img src="assets/imgs/gallery/image-28.webp" alt="image" data-speed="0.8"></div>
                <div class="image parallax-view"><img src="assets/imgs/gallery/image-29.webp" alt="image" data-speed="0.8"></div>
              @endif
            </div>
          ...
          <div class="container large">
            <div class="pagination fade-anim">
              <a href="{{ route('front.projects.index') }}"><svg width="13" height="18" viewBox="0 0 13 18" fill="none"
                ...
                Prev</a>
              <a href="{{ route('front.projects.index') }}">Next<svg width="13" height="18" viewBox="0 0 13 18" fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd"
