  ...
  <section class="work-details-area">
      ...
      <div class="container large">
        ...
        <div class="meta-wrapper fade-anim">
          <div class="meta-item">
            ...
            <p class="text">{{ $project->category ?: 'General' }}</p>
          </div>
          <div class="meta-item">
            <p class="title">Client</p>
            <p class="text">{{ $project->client_name }}</p>
          </div>
          <div class="meta-item">
            <p class="title">Date</p>
            <p class="text">{{ optional($project->execution_date)->format('F Y') ?? $project->created_at->format('F Y') }}</p>
          </div>
          <div class="meta-item">
            <p class="title">Status</p>
            <p class="text">{{ ucfirst($project->status) }}</p>
          </div>
        </div>
      </div>
          ...
          <div class="content">
            ...
            <div class="feature-list">
              <ul>
                ...
                <li>Published: {{ optional($project->execution_date)->format('Y') ?? $project->created_at->format('Y') }}</li>
                <li>Status: {{ ucfirst($project->status) }}</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div class="gallery-wrapper fade-anim">
        @forelse($galleryItems as $media)
          <div class="image parallax-view">
            <img src="{{ $media->getUrl() }}" alt="{{ $project->name }}" data-speed="0.8">
          </div>
        @empty
          <div class="image parallax-view">
    ...
    <div class="container large">
      <div class="pagination fade-anim">
        @if($previousProject)
          <a href="{{ route('front.projects.show', $previousProject) }}"><svg width="13" height="18" viewBox="0 0 13 18" fill="none"
            ...
            Prev</a>
        @else
          <a href="{{ route('front.projects.index') }}">All Projects</a>
        @endif

        @if($nextProject)
          <a href="{{ route('front.projects.show', $nextProject) }}">Next<svg width="13" height="18" viewBox="0 0 13 18" fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd"
