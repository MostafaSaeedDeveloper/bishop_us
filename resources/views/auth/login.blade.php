...
<div class="mb-2 text-center"><a class="link-fx fw-bold fs-1" href="{{ route('front.home') }}"><span class="text-dark">Bishop</span><span class="text-primary">CMS</span></a><p class="text-uppercase fw-bold fs-sm text-muted">Sign In</p></div>
<form method="POST" action="{{ route('login') }}">@csrf
<div class="mb-4"><div class="input-group input-group-lg"><input type="email" class="form-control" name="email" value="{{ old('email') }}" placeholder="Email" required autofocus><span class="input-group-text"><i class="fa fa-user-circle"></i></span></div>@error('email')<small class="text-danger">{{ $message }}</small>@enderror</div>
